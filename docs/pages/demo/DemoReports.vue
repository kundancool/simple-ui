<template>
    <div>
        <DemoStage title="Live occupancy report">
            <s-page-header title="September report" subtitle="Generated just now · all properties.">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <s-stat-card :icon="BedDouble" bg-color="s-bg-icon-blue" icon-color="s-text-icon-blue" value="86%" label="Occupancy" />
            ...
            <div class="grid lg:grid-cols-2 gap-3 mt-3">
                <s-card title="Revenue by channel">
                    <s-bar-chart :labels="['Direct', 'MMT', 'B.com', 'Goibibo']" :datasets="[{ label: '₹k', data: [120, 210, 180, 60], backgroundColor: '#0c7076', borderRadius: 4 }]" :height="220" />
                </s-card>
                <s-card title="Top room types">
                    <s-data-table :data="rooms" :stripe="false" size="small">
                        <s-data-table-column prop="type" label="Room type" />
                        <s-data-table-column prop="nights" label="Nights" align="right" width="90px" />
...
<script setup>
import { BedDouble, Wallet, TrendingUp, Star } from 'lucide-vue-next'
...
const { info } = useToast()
const rooms = [
    { type: 'Deluxe', nights: 412, revenue: '₹1.9L' },
    { type: 'Suite', nights: 188, revenue: '₹1.4L' },
    { type: 'Standard', nights: 356, revenue: '₹0.9L' },
]
