<template>
    <div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
            ...
            <s-stat-card :icon="CalendarCheck" bg-color="s-bg-icon-blue" icon-color="s-text-icon-blue" :value="kpiOrders" label="Orders" />
            <s-stat-card :icon="Percent" bg-color="s-bg-icon-purple" icon-color="s-text-icon-purple" :value="kpiOcc" label="Utilization" />
        ...
        <div class="grid lg:grid-cols-2 gap-3 mt-3">
            <s-card title="Orders by Channel">
                <s-bar-chart :labels="channels" :datasets="otaData" :height="240" />
            </s-card>
...
const otaData = computed(() => [{ label: 'Orders', data: varied([42, 68, 55, 21]), backgroundColor: '#0c7076', borderRadius: 4 }])
...
const kpiOrders = computed(() => String(128 + seed.value * 9))
