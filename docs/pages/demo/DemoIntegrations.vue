<template>
    <div>
        <DemoStage title="Live integrations">
            <s-page-header title="Integrations" subtitle="Connect the tools around your properties." />
...
const apps = ref([
    { name: 'Slack alerts', blurb: 'Overbooking pings', icon: MessageSquare, bg: 's-bg-icon-purple', fg: 's-text-icon-purple', on: true },
    { name: 'Razorpay', blurb: 'Card + UPI collection', icon: CreditCard, bg: 's-bg-icon-blue', fg: 's-text-icon-blue', on: true },
    { name: 'Mailgun', blurb: 'Guest emails', icon: Mail, bg: 's-bg-icon-yellow', fg: 's-text-icon-yellow', on: false },
