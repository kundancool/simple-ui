        ...
        <s-auth-layout title="Acme Inc" subtitle="Workspace admin console">
            <s-card>
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <s-button class="w-full" :loading="busy" @click="submit">{{ mode === 'login' ? 'Sign in' : 'Create account' }}</s-button>
                    <s-alert v-if="done" variant="success" :title="mode === 'login' ? 'Welcome back' : 'Account created'">This is a demo — no request leaves the browser.</s-alert>
                </div>
            </s-card>
